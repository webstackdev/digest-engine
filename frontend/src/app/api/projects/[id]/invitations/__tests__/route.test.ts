...
describe("POST /api/projects/[id]/invitations", () => {
  ...
  it("defaults the role to member and the redirect to the invite page", async () => {
    vi.mocked(createProjectInvitation).mockResolvedValue({
      id: 3,
      project: 4,
      email: "invitee@example.com",
    ...
    await expect(getLocation(response)).resolves.toBe(
      "http://localhost/projects/4/members/invite?message=Invitation+sent.",
    )
  })

  it("redirects with the fallback error when a non-Error value is thrown", async () => {
    vi.mocked(createProjectInvitation).mockRejectedValue("boom")
    ...
    await expect(getLocation(response)).resolves.toBe(
      "http://localhost/projects/4/members/invite?error=Unable+to+send+invitation.",
    )
  })
})
