  ...
  return (
    ...
    >
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between gap-4 rounded-3xl border border-ink/12 bg-surface/85 p-5 shadow-panel backdrop-blur-xl">
          <div>
        ...
        <article className="space-y-4 rounded-3xl border border-ink/12 bg-surface/85 p-5 shadow-panel backdrop-blur-xl">
          <div className="space-y-1">
            ...
            <h3 className="m-0 font-display text-title-sm font-bold text-ink">Memberships</h3>
          </div>
          <div className="space-y-3">
            {memberships.map((membership) => (
              <article
                className="grid gap-4 rounded-2xl border border-ink/10 bg-surface-strong/45 p-4 lg:grid-cols-[minmax(0,1fr)_auto_auto] lg:items-center"
                key={membership.id}
              >
                <div>
                ...
                <form action={`/api/projects/${selectedProject.id}/members/${membership.id}`} className="flex flex-wrap items-center gap-3" method="POST">
                  ...
                  <select
                    ...
                    className="rounded-2xl border border-ink/12 bg-surface px-4 py-3 text-sm text-ink outline-none transition focus:border-primary/40 focus:ring-2 focus:ring-primary/15"
                  ...
                  <button
                    className="inline-flex min-h-11 items-center justify-center rounded-full border border-ink/12 bg-transparent px-4 py-3 text-sm font-medium text-ink transition hover:bg-surface-strong/65"
                    type="submit"
                  >
                    Update role
                  </button>
                ...
                <form action={`/api/projects/${selectedProject.id}/members/${membership.id}`} method="POST">
                  ...
                  <button
                    className="inline-flex min-h-11 items-center justify-center rounded-full border border-danger/25 bg-danger/10 px-4 py-3 text-sm font-medium text-danger-ink transition hover:bg-danger/16"
                    type="submit"
                  >
                    Remove
                  </button>
        ...
        <article className="space-y-4 rounded-3xl border border-ink/12 bg-surface/85 p-5 shadow-panel backdrop-blur-xl">
          <div className="space-y-1">
            ...
            <h3 className="m-0 font-display text-title-sm font-bold text-ink">Invitations</h3>
          </div>
          {invitations.length === 0 ? (
            ...
            <div className="space-y-3">
              {invitations.map((invitation) => (
                <article
                  className="grid gap-4 rounded-2xl border border-ink/10 bg-surface-strong/45 p-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center"
                  key={invitation.id}
                >
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                      <a className="text-sm font-medium text-primary hover:text-primary-strong" href={invitation.invite_url}>
