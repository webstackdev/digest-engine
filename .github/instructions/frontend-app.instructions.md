...
- Prefer semantic Tailwind theme utility classes such as `bg-background`, `bg-card`, `bg-primary`, `text-foreground`, `text-muted-foreground`, `text-primary-foreground`, or `border-border` when the color should change with the active light or dark theme.
...
- Do not use slash-opacity color utilities such as `bg-primary/50`, `border-border/10`, `text-foreground/80`, or `ring-ring/40`; choose the closest existing semantic or fixed palette class instead.
