...
from projects.model_support import SourcePluginName, normalize_bluesky_handle
...
class BlueskySourcePlugin(SourcePlugin):
    ...
    def validate_config(cls, config: object) -> dict:
        ...
        if author_handle:
            normalized_handle = normalize_bluesky_handle(author_handle)
    ...
    def match_entity_for_item(self, item: ContentItem):
        ...
        if author_handle:
            for entity in self.project.entities.exclude(bluesky_handle=""):
                if normalize_bluesky_handle(entity.bluesky_handle) == author_handle:
                    return entity
